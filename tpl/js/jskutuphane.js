function sspot_arastir(t) {
    document.write('<select onchange="if(this.selectedIndex>0){window.open(decodeURIComponent(this.options[this.selectedIndex].value)+\'' + t + '\');this.selectedIndex=0;}">');
    document.write(
    '<option>..bilgi kaynakları..</option>' +
    '<option value="http://sozluk.sourtimes.org/show.asp?t=">ek�?i sözlük</option>'+
    '<option value="http://www.uludagsozluk.com/k/">uluda�? sözlük</option>' +
...
function koy(id,bas,son) {
	...
	if (document.selection){
		...
		if(el!=t){
			alert('metni seç, sonra tu�?a bas.');
		}else{
	...
	function entry_td(a,href,title,id){
		...
		if(action!='self' && action!='ogse' && action!='okur'){
			entry_td(':)','" onclick="return oyp(\'1\','+id+');','�?ükela',id);
			entry_td(':o','" onclick="return oyp(\'0\','+id+');','orta',id);
			entry_td(':(','" onclick="return oyp(\'-1\','+id+');','�?aibeli',id);
